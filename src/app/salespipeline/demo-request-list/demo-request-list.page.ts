...
export class DemoRequestListPage implements OnInit, OnDestroy {
  ...
  async sendDemoEmail(reqId) {
    ...
    this.demoRequestService.getDemoRequestById(reqId).subscribe((res) => {
     console.log(res.accInstallation.toString());
      var emailobj = {
        to: approvers,
  ...
  async sendConfirmationEmail(userId,reqId,comment,stflag) {
    let status=stflag==1?"Approved":"Rejected";
    let user = await this.demoRequestService.getUserByEmail(userId);
    let userName = await this.authService.userName.pipe(first()).toPromise();
    var emailobj = {
      to: user.email,
      from: 'jyoti.gogave@dwijafoods.com',
      subject: `${status}:Demo request ${reqId}`,
      raisedBy: userName,
      request: comment,
      flag:stflag,
    };
              ...
              this.loadingCtrl
                ...
                .then(async (loadingEl) => {
                  ...
                  this.demoRequestService
                    ...
                    .subscribe(() => {
                      this.sendConfirmationEmail(request.userId,request.id, element.value != undefined ? element.value : '',1);
                      loadingEl.dismiss();
                    });
                });
            },
              ...
              this.loadingCtrl
                ...
                .then((loadingEl) => {
                  ...
                  this.demoRequestService
                    ...
                    .subscribe(() => {
                      this.sendConfirmationEmail(request.userId,request.id, element.value != undefined ? element.value : '',2);
                      loadingEl.dismiss();
                    });
                });
            },
