...
import { ClientExpetions, MastBranch } from 'src/app/models/division.model';
import { DivisionService } from 'src/app/services/division.service';
import { DatePipe } from '@angular/common';
import { convertToDateTime, convertTimeStampToDate, getActiveYear } from 'src/app/utilities/dataconverters';
...
export class InvoiceListPage implements OnInit {
  ...
  _clientexceptions:ClientExpetions[]=[];

  @ViewChild('searchElement') searchElement;

  constructor(private salespiplineService: SalespipelineService,
 ...
 async ngOnInit() {
  ...
  async  createInvoice(req,invoice:Invoice,taxType){
    ...
    let branch: MastBranch = await(
      await this.divisionService.getBrancheByName(invoice.billbranch)
    )[0];
    ...
    nbody.push([
      ...
      {
        colSpan: 2,
        text: `Invoice No.:\n${new Date(demodata.date)>new Date(2021, 11, 6)?invoice['invNo']:invoice['srNo']}\n`,
        fontSize: 8,
      },
      '',
      {
  ...
  async  createRetalInvoice(req,invoice:Invoice,taxType){
    let branch: MastBranch = await(
  ...
  let rentSrNo:string="";
   if(rentInv.length>0){
    ...
    rentinvoice.month=invoice.month;
    rentinvoice.displaymonth=invoice.displaymonth;
    rentinvoice.clientId=req.clientId;
    rentinvoice.branch=branch.name;
    rentinvoice.clientName=req.billName;
    rentinvoice.clientLocationId=req.locationId;
    rentinvoice.clientLocation=req.location;
    rentinvoice.taxType=taxType;
    rentinvoice.mchRent=invoice.mchRent,
    rentinvoice.mchdeposite=invoice.mchdeposite,
    rentinvoice.mchinstCharges=invoice.mchinstCharges,
...
deleteInvoice(inv:Invoice){
  this.alertCtrl.create({
    ...
    message:
      '<strong>Are you sure you want delete this invoice '+ inv.srNo+'?</strong>',
      inputs: [
        {
          name: 'chkfreeDC',
          label: 'Unplug DCs from Invoice?',
          type:'checkbox',
          value: 'true',
          checked: false
        },
    ...
    buttons: [
      ...
      },
      {
        ...
        handler:async (data) => {
          ...
          inv.isDeleted=true;
          await this.salespiplineService.deleteInvoice(inv,unplughdc).then(async x=>{
            this.invoicelist.splice(this.invoicelist.indexOf(inv),1);
            this.filterinvoicelist.splice(this.filterinvoicelist.indexOf(inv),1);

            const toast = await this.toastController.create({
              message: inv.srNo+ ' Invoice deleted',
              duration: 2000,
              color: 'success',
            });
            toast.present();
...
chkInvoiceChange(event,invoice){
  ...
  this.invoiceTotal= 0;
   ...
   invoices.forEach(invoice => {
    let invamt= (isNaN(invoice.amount)?0:invoice.amount);
    let invtax= (isNaN(invoice.tax)?0:invoice.tax);
    this.invoiceAmount=this.invoiceAmount +invamt;
    this.invoiceTax=this.invoiceTax+ invtax ;
    this.invoiceTotal=this.invoiceTotal+ (invamt+invtax);
   });
...
async applyFilter(){
  ...
  this.invoicelist.forEach((inv) => {
     if(inv.srNo.toLowerCase().indexOf(serchTerm.toLowerCase())> -1
    ) {
      inv['isChecked']=false;
      filterinvoices.push(inv);
    }
    else if(serchTerm.toLowerCase()=="machine" && inv.srNo.indexOf("/M")>-1){
      inv['isChecked']=false;
      filterinvoices.push(inv);
    }
  ...
  if(filterinvoices.length<=0 && serchTerm==""){
    this.invoicelist.forEach((inv) => {
      inv['isChecked']=false;
    });
...
getInvoiceNumber(invoice:Invoice){
  return new Date(convertTimeStampToDate(invoice.createdOn))>new Date(2021, 11, 6)?invoice.invNo:invoice.srNo;
}
  getBase64Image(){
