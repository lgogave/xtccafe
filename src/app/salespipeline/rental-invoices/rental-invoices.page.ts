...
import { ClientSalesPipeline, DCMaterial, InvoiceMonth, ReceiptBook, RentalInvoice,Location, BillingDetail } from '../salespipeline.model';
...
async loadMonths(){
 ...
 async generateInvoice(rentInvoice:RentalInvoice[]){
  ...
  let rentSrNo=invoice.srNo;
  let branch: MastBranch = await(
  await this.divisionService.getBrancheByName(req['branch']))[0];
...
chkInvoiceChange(event,sale){
  ...
  sale.locations.forEach(loc => {
    loc['rentalInvoice']?.forEach((inv,index) => {
      this.invoiceAmount=this.invoiceAmount+inv.mchRent;
      this.invoiceTax=this.invoiceTax+(inv.mchRent*0.18);
