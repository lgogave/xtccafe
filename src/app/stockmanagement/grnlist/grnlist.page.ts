...
import { GrnrequisitionService } from '../grnrequisition/grnrequisition.service';
import { GRNRequistionRequest } from '../grnrequisition/grnrequisition.model';
...
import { ConvertDateToMMMYYYY } from 'src/app/utilities/dataconverters';
pdfMake.vfs=pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-grnlist',
  templateUrl: './grnlist.page.html',
  styleUrls: ['./grnlist.page.scss'],
})
export class GrnlistPage implements OnInit {
  isLoading = false;
  reqId:string=null;
  grnlist: GRNRequistionRequest[];
  filtergrnlist: GRNRequistionRequest[];
  pdfObj=null;
  reqFrom=null;
  poId=null;
  id=null;
...
this.route.paramMap.subscribe(
  ...
  async doRefresh(event) {
    this.isLoading = true;
   if(this.reqFrom && this.reqFrom.toLowerCase()=="po"){
    this.poId=this.id;
    this.grnlist = await this.grnreqService.getByPOId(this.id);
   }
   else if(this.reqFrom && this.reqFrom.toLowerCase()=="grn"){
    this.reqId=this.id;
    this.grnlist = await this.grnreqService.getById(this.id);
   }
   else{
    this.grnlist = await this.grnreqService.getAll();
   }
    this.filtergrnlist = await this.applyFilter();
  ...
  navigateToAdd(){
      this.router.navigate(['/','stockmanagemt','grnrequisition',this.poId,0]);
  }
  navigateToEdit(req){
    this.router.navigate(['/','stockmanagemt','grnrequisition',this.poId,req.id]);
}
async deleteGRNRequest(req:GRNRequistionRequest){
  this.alertCtrl
    .create({
      header: 'Delete Invoice!',
      message: '<strong>Are you sure you want delete this GRN ?</strong>',
      buttons: [
        {
          text: 'Cancel',
          role: 'cancel',
        ...
        },
        {
          text: 'Okay',
          handler: async (data) => {
            let poreqDetail = await this.poreqService.getById(req.poId);
            req.materialDetails.forEach((material, index) => {
              poreqDetail.materialDetails[index].actQty =
                poreqDetail.materialDetails[index].actQty +
                material.actQtyRecived;
            });
            req.isDelete = true;
            await this.grnreqService.addupdate(req, true).subscribe((res) => {
              this.toastController
                ...
                .then(async (tost) => {
                  let stockreg = <StockRegister>Object.assign({}, req);
                  ...
                  tost.present();
                  this.router.navigate(['/stockmanagemt/grnlist/' + this.poId]);
                });
            });
  ...
  async applyFilter(){
    ...
    let filterresult: GRNRequistionRequest[] = [];
    this.grnlist.forEach((po) => {
